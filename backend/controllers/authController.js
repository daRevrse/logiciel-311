...
async function registerByEmail(req, res) {
  try {
    ...
    const { email, password, full_name, municipality_id, municipality_slug } = req.body;

    let municipalityId = municipality_id ? parseInt(municipality_id, 10) : null;
    if (!municipalityId && municipality_slug) {
      const m = await Municipality.findOne({ where: { slug: municipality_slug, is_active: true }, attributes: ['id'] });
      if (!m) return res.status(404).json({ success: false, message: 'Municipalité introuvable' });
      municipalityId = m.id;
    }
    if (!municipalityId) {
      return res.status(400).json({ success: false, message: 'municipality_id ou municipality_slug requis' });
    }
    ...
    // L'envoi d'email de vérification est best-effort (à connecter plus tard)
    res.status(201).json({
      success: true,
      message: 'Compte créé',
      data: {
    ...
    logger.error('Erreur register email:', error);
...
async function loginByEmail(req, res) {
  try {
    ...
    logger.error('Erreur login email:', error);
...
const emailValidationRules = {
  register: [
    ...
    body('full_name').trim().isLength({ min: 2, max: 255 }).withMessage('Nom complet requis')
  ],
  login: [
    body('email').isEmail().withMessage('Email invalide'),
